...
const int AGE_BITS = 10;         // Number of bits for age counter
...
const int PERIOD_UPDATES = 5000;  // Number of bits for timestamp counter
...
double opportunityCost[MAX_AGE_VALUE] = {0.0};
double hitProbability[MAX_AGE_VALUE] = {0.0};
double expectedLifetime[MAX_AGE_VALUE] = {0.0};
...
//     std::vector<double> events( MAX_AGE_VALUE );
//     std::vector<double> eventsR( MAX_AGE_VALUE );
//     std::vector<double> eventsNR( MAX_AGE_VALUE );
//     std::vector<double> totalEventsAbove( MAX_AGE_VALUE + 1 );
//     totalEventsAbove[ MAX_AGE_VALUE ] = 0.;
//     for (int age = MAX_AGE_VALUE -1; age >= 0; age--) {
//     absl_hitCtrs_nn_sum += absl_hitCtrs_nn[age][0];
//     absl_hitCtrs_nn_sum += absl_hitCtrs_nn[age][1];
//     absl_evictionCtrs_nn_sum += absl_evictionCtrs_nn[age][0];
...
//     // hNR[age] << std::endl;
//   }
//     for (uint32_t age = MAX_AGE_VALUE - 1; age < MAX_AGE_VALUE; age--) {
//         eventsR[age] = absl_hitCtrs_nn[age][1] + absl_evictionCtrs_nn[age][1];
//         eventsNR[age] = absl_hitCtrs_nn[age][0] + absl_evictionCtrs_nn[age][0];
//     events[age] = eventsR[age] + eventsNR[age];
//         totalEventsAbove[age] = totalEventsAbove[age+1] + eventsR[age] + eventsNR[age];
//     absl_hit[age] = absl_hitCtrs_nn[age][1] + absl_hitCtrs_nn[age][0];

//     }

//     uint32_t a = MAX_AGE_VALUE - 1;
//     hitProbability[a] = (totalEventsAbove[a] > 1e-2)? 0.5 * absl_hit[a] / totalEventsAbove[a] : 0.;
//     expectedLifetime[a] = a; // temporary was age scalling E[L =a]
//     double expectedLifetimeUnconditioned = a * totalEventsAbove[a];
//     double totalHitsAbove = absl_hit[a];

//     // short lines
//     //
//     // computed assuming events are uniformly distributed within each
//     // coarsened region.
//     for (uint32_t a = MAX_AGE_VALUE - 2; a < MAX_AGE_VALUE; a--) {
//         if (totalEventsAbove[a] > 1e-2) {
//             hitProbability[a] = (0.5 * absl_hit[a] + totalHitsAbove) / (0.5 * events[a] + totalEventsAbove[a+1]);
//             expectedLifetime[a] = ((1./6) * a * events[a] + expectedLifetimeUnconditioned) / (0.5 * events[a] + totalEventsAbove[a+1]);
...
//         totalHitsAbove += absl_hit[a];
//         expectedLifetimeUnconditioned += a * totalEventsAbove[a];
//     }
//   double lineGain = absl_hitCtrs_nn_sum / (absl_hitCtrs_nn_sum + absl_evictionCtrs_nn_sum) / (NUM_SETS * NUM_WAYS);
//     // finally, compute EVA from the probabilities and lifetimes
//     for (uint32_t a = MAX_AGE_VALUE - 1; a < MAX_AGE_VALUE; a--) {
//         if (unlikely(std::isnan(lineGain))) {
//             opportunityCost[a] = 0.;
//         } else {
//             opportunityCost[a] = lineGain * expectedLifetime[a];
//         }
//         G_eva[0][a] = hitProbability[a] - opportunityCost[a];
//     }   
//   return G_eva;
// }
Cache_Age cache_age;
...
void update_replacement_state(uint32_t set, uint32_t way, uint8_t hit, uint32_t type) {
  unsigned int ageCounter;
  bool classificationBit;
  unsigned int timestamp;
  // Get cache line info
...
if (accessCount % 10 == 0 && accessCount >= 100) {
    ...
    if (accessCount % PERIOD_UPDATES == PERIOD_UPDATES - 1) {
